...
var startMsi = function(){
    ...
    module_code = jQuery("#module_code").val() + '_';
    
    createPaymentForm([]);
    jQuery('#'+module_code+'msi').prop( "disabled", true );

    jQuery("input[name='payment[numero_tarjeta]']").on('blur', function () {
        jQuery('#'+module_code+'msi').prop( "disabled", true );
...
var getConfiguration = function ( creditCardNumber, branchId, companyId ) {
    ...
    jQuery.ajax({
        ...
        error: function (xhr, status, error) {
            jQuery("#select_msi").prop( "disabled", true );
        }
    });
}
...
var validatePayment = function ( month ) {
    ...
    let amount = parseFloat( jQuery("input[name='monto']").val() );
